...
Result: failed because `reader_finished.wait(timeout=0.1)` returned `True` while
the writer transaction was still open.
...
Result: `.omo/evidence/prd-remainder-task-4-read-only.json` reports `ok: true`,
`changed_paths: []`, and `network_attempts: 0`.
...
Cleanup: tmux session `ulw-qa-prd-4` exited; `tmux has-session -t ulw-qa-prd-4`
returned 1.
